...
from faker2.config import AVAILABLE_LOCALES
from faker2.config import PROVIDERS as STANDARD_PROVIDER_NAMES
...
def _write_base_provider_docs():
    ...
    with (DOCS_ROOT / "providers" / "baseprovider.rst").open("wb") as fh:
        _hide_edit_on_github(fh)
        _write_title(fh, "``faker2.providers``")
        _write_includes(fh)
        _write(
            fh,
            PROVIDER_AUTODOC_TEMPLATE.format(
                provider_class="faker2.providers.BaseProvider",
